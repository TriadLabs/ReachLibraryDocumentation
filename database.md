...
| VolumeSegment                  | --(Not currently in use.) This table is used to store the individual x,y,z coordinates that make up a VolumeSegmentSequence. This was used to create custom motion paths that were user genereated in the Reach App. This table will be phased out soon.  |
| VolumeSegmentSequence          | --(Not currently in use.) This table is used to record custom motion sequences in the Reach App. It records the metadata associated with a single recorded sequence but not the actual points that compose the sequence itself. |
